...
# X <- unnest_tokens(train, 'splitted', 'text', token="words") %>%
...
# 
# X = data.frame(X[-c(1)], row.names = X$splitted)
```
       ...
       methods = list(
                    ...
                    predict = function(message)
                    {
                      ...
                      for (author in 1:3) {
                        ...
                        for (idx in 1:length(string_arr)) {
                          
                          if (!(string_arr[idx] %in% splitted_stop_words)) {
                            word_count <- tidy_X[string_arr[idx], author]
                    ...
                    {
                      correct_guesses = 0
                      data_set_length = length(X_test$text)

                      for (i in 1:length(X_test$text)) {
                        result_of_prediction = predict(X_test$text[i])

                        if (author_count[result_of_prediction, 1] == y_test$author[i]) {
                          correct_guesses = correct_guesses + 1
                        }
                      }

                      print(correct_guesses / data_set_length)
                    }
))
